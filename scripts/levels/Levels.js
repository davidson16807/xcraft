...
class Curriculum {
    constructor(courses, levels) {
        typecheck(courses, 'Array');
        typecheck(levels, 'Array');
...
function vanilla_curriculum(grouplikes) {
    ...
    /* 
    `nested` is a nested representation where levels are nested within courses
    it simplifies the problem of level and course specification.
    `nested` is converted to a flat representation 
    that simplifies code for traversing and rendering lessons and courses
    */
    const nested = [
        {
            title: 'Arithmetic', 
    ...
    nested.forEach(course => {
        const first_level_index = lessons.length;
        const course_lessons = course.lessons.map((level, index) => freeze({
            index: first_level_index + index,
            title: level.title,
            concept: level.concept,
            equation: level.equation,
            goal: level.goal,
        }));
        lessons = [...lessons, ...course_lessons];
    ...
    return new Curriculum(courses, lessons);
}
