...
function Equations(dependencies) {
    ...
    */
    ...
    function operation_for_source(equation, source_path) {
        ...
        const root = paths.resolve(equation, parsed.side);
        if (root == null || root.type !== 'add') return null;

        if (parsed.segments.length === 1) return 'add';
        if (parsed.segments.length === 2 && root.contents.length === 1) return 'mul';
        return null;
    }
    ...
    function balance(equation, source_path, target_side) {
        ...
        if (operation === 'add') {
            ...
            new_source = expressions.remove(source_root.contents[0], index);
    ...
    function swap(equation, path1, path2) {
        ...
        const replacement = expressions.swap(
            parent,
            Number(paths.segment(path1)),
            Number(paths.segment(path2))
        );
